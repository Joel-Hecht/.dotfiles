...
#This technically falls under the scope of make_symlinks.txt, but since other dmenu stuff is dealt with in this file, I have added it here instead
#make_aliases.sh executes before make_symlinks.sh in normal run order, so we need to make all the files here that will be symlinked in the future.  The issue then is that all the symlinks will not be made, so we need to reference the dotfiles filesystem
